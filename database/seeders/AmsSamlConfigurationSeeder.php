...
class AmsSamlConfigurationSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        DB::transaction(function () use ($certificate) {
            ...
            $configuration = SamlConfiguration::query()
                ->where('slug', 'ams-upcebu-idp')
                ->orWhere('entity_id', self::ENTITY_ID)
                ->firstOrNew();
    ...
    private function certificate(): string
    {
        ...
        if (str_contains($certificate, 'PASTE_AMS_CERTIFICATE_HERE')) {
            throw new RuntimeException('Replace SAML_IDP_PUBLIC_CERT placeholder with the real AMS IdP signing certificate.');
        }
